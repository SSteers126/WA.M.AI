...
def generate_boolean_label_df(label_list: list[tuple[str, str]], columns: tuple | list) -> DataFrame:
    """
    ...
    :return: A new `DataFrame` with each row containing a single sample, and a set of False labels
    """

    # Each row begins as the sample file name, and each label set to `False`
    row_list = ((dataset_sample[1], *(False for _ in range(len(columns) - 1))) for dataset_sample in label_list)
